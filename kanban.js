...
// ==========================================================================
// CORE LOGIC - moves tasks between columns
// ==========================================================================
function mutateTaskPipelineStatus(taskId, targetStatus) {
    ...
    // BLOCK: unassigned user tries to interact
    if (!isMyTask && !isAdmin) {
    ...
    if (isMyTask && !isAdmin) {
        ...
        // review → doing (revert back to work before admin decides)
    ...
    if (isAdmin) {
        ...
        // review → rejected (reject with reason, goes back to doing)
        ...
        // rejected → doing (send back to work)
        ...
        // assigned → doing (admin starts task) - REMOVED per spec: only assigned user can accept
        // if (task.status === 'assigned' && targetStatus === 'doing') {
        //     ...
        // }
    ...
    ['assigned', 'doing', 'review', 'done', 'rejected'].forEach(trackName => {
        ...
        tracks[trackName].forEach(task => {
            ...
            if (isAdmin) {
                ...
                // Admin has NO action buttons in assigned column - only assigned user can accept/reject
            }
