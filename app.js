...
// Core Background Chronos Execution Tracker - Placeholder implementation
function checkActiveMilestonesEngine() {
    // This function is intended to check and update milestone statuses
    // For now, we'll just log that it's running to confirm it's working
    console.debug('checkActiveMilestonesEngine executed');
}
...
const SYSTEM_LOCALIZATION_DICTIONARY = {
    en: {
        authTitle: 'نظام إدارة المهام مع <span class="pro-badge">ismail</span>',
        authSubtitle: 'منصة متكاملة لإدارة المهام والعمليات',
...
async function loadCoreRuntimeStorage() {
    ...
    // 시도: Firebase에서 상태 로드 (Firebase가 실제 데이터が있을 때만 사용)
    let firebaseHasData = false;
    try {
        const remoteState = await fetchRemoteState();
        if (remoteState && Array.isArray(remoteState.types) && remoteState.types.length > 0) {
            Object.assign(state, remoteState);
            console.log('State loaded from Firebase with', remoteState.types.length, 'modules');
    ...
    // localStorage 로드 (Firebase가 비어있거나 실패하면 여기서 로드)
    if (!firebaseHasData) {
        ...
        if (hydrator) {
            try {
                ...
                if (parsed.types && parsed.types.length > 0) {
                    ...
                    console.log('State loaded from localStorage with', parsed.types.length, 'modules');
                    loaded = true;
                }
            } catch (e) {
    ...
    if (!state.accessRegistry[ROOT_ADMIN_ID]) {
        state.accessRegistry[ROOT_ADMIN_ID] = { name: "Admin", role: "System Administrator" };
    }
...
function seedInitialOperationalData() {
    ...
    state.tasks = [
        { id: "stsk1", typeId: "st1", sprintId: "sp1", name: "Configure Pipeline Tokens", assignedTo: ["Admin"], assignedUser: "Admin", status: "assigned", dueDate: "2026-12-31T23:59", notes: "Use standard security keys", tags: "security, api", customColor: "#4a6cf7" }
    ];
...
function showLoginNotification() {
    ...
    if (isAdmin) {
        tasksNeedingAction = state.tasks.filter(t => t.status === 'assigned' || t.status === 'review');
    } else {
        tasksNeedingAction = state.tasks.filter(t => t.assignedUser === state.currentUser && (t.status === 'assigned' || t.status === 'review'));
    }
...
function applyLanguageDOMEngine() {
    ...
    if (langLabel) langLabel.innerText = state.currentLanguage === 'en' ? 'AR' : 'EN';
...
if (targetViewId === 'dashboard') {
                if (typeof renderDashboardSummaryHub === "function") renderDashboardSummaryHub();
...
function hydrateUserOptionSelectors() {
    const select = document.getElementById("taskAssignedUsers");
    if(!select) return;
    select.innerHTML = "";
...
function handleModuleSubmit(event) {
    ...
    if (!name) {
        showToast(state.currentLanguage === 'ar' ? 'يرجى إدخال اسم Project' : 'Please enter module name', 'warning');
        return;
    }
...
function triggerDeleteProject(projectId) {
    ...
    const confirmMsg = state.currentLanguage === 'ar' ? "هل أنت متأكد من حذف هذا Project نهائياً مع كافة المهام التابعة له؟" : "Are you sure you want to delete this project and all related tasks?";
...
function renderSharedModulesStackList() {
    ...
    const dict = isRTL ? {
        ...
        noSprints: 'لا توجد سبرينتات داخل هذا Project',
    ...
    state.types.filter(t => !t.isPrivate).forEach(moduleType => {
        ...
        moduleSprints.forEach(sprint => {
            ...
            sprintTasks.forEach(task => {
                ...
                tasksHTML += `
                    <div style="border-top: 3px solid ${taskColor}; margin: 6px 0; padding: 8px; background: var(--bg-surface); border-radius: 4px; border-${isRTL ? 'right' : 'left'}: 2px solid ${(task.assignedTo && task.assignedTo.includes(state.currentUser)) || task.assignedUser === state.currentUser ? 'var(--primary)' : 'transparent'};">
                        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 12px;">
                            <div>
                                <span style="${isDone ? 'text-decoration: line-through; opacity: 0.6;' : ''}">${task.name}</span>
                                <small style="display: block; color: var(--text-muted); font-size: 11px;">${(task.assignedTo && task.assignedTo.length > 0 ? task.assignedTo.join(', ') : 'Unallocated')} • ${task.status}</small>
                            </div>
...
<div class="sprint-card" style="border: 2px dashed ${sprint.color}; border-radius: 8px; padding: 12px; margin: 12px 0; cursor: pointer;" onclick="this.classList.toggle('expanded');">
                          ...
                          ${isAdmin ? `<div style="display: flex; gap: 4px;" onclick="event.stopPropagation();">
                              <div style="position: relative; display: inline-block;">
                                  ...
                                  <div id="sprintActions_${sprint.id}" class="private-dropdown-menu" style="display: none; position: absolute; ${isRTL ? 'left' : 'right'}: 0; top: 100%; min-width: 100px;">
                                      <button onclick="event.stopPropagation(); createTaskForSprint('${sprint.id}')" style="display: block; width: 100%; padding: 6px 10px; text-align: ${isRTL ? 'right' : 'left'}; border: none; background: none; cursor: pointer; font-size: 11px;"><i class="fa-solid fa-plus" style="color: var(--success); margin-${isRTL ? 'left' : 'right'}: 4px; font-size: 10px;"></i> ${dict.createTask}</button>
            ...
            directTasksHTML += `
                <div style="border-top: 3px solid ${taskColor}; margin: 6px 0; padding: 8px; background: var(--bg-surface); border-radius: 4px; border-${isRTL ? 'right' : 'left'}: 2px solid ${isMyTask ? 'var(--primary)' : 'transparent'};">
                    <div style="display: flex; justify-content: space-between; align-items: center; font-size: 12px;">
                        <div>
                            <span style="${isDone ? 'text-decoration: line-through; opacity: 0.6;' : ''}">${task.name}</span>
                            <small style="display: block; color: var(--text-muted); font-size: 11px;"><span style="color: ${task.dueDate && new Date(task.dueDate) < new Date() && task.status !== 'done' ? 'var(--danger)' : ''};"><i class="fa-solid fa-clock"></i> ${task.dueDate ? task.dueDate.split("T")[0] : 'Open Deadline'}</span> • ${task.assignedUser || 'Unallocated'} • ${task.status}</small>
                        </div>
                        ${isAdmin && isDirectTaskModuleActive ? `<div style="display: flex; gap: 2px; align-items: center;">
                            <button onclick="event.stopPropagation(); toggleTaskActivation('${task.id}')" style="background: ${task.isActive ? 'var(--success)' : 'var(--warning)'}; border: none; color: #fff; width: 22px; height: 22px; border-radius: 3px; cursor: pointer; font-size: 11px;" title="${task.isActive ? (isRTL ? 'إلغاء التفعيل' : 'Deactivate') : (isRTL ? 'تفعيل' : 'Activate')}"><i class="fa-solid fa-power-off"></i></button>
                            <div style="position: relative; display: inline-block;">
                                <button onclick="event.stopPropagation(); toggleTaskActions('${task.id}')" style="background: var(--text-muted); border: none; color: #fff; padding: 2px; border-radius: 3px; cursor: pointer; font-size: 10px;"><i class="fa-solid fa-ellipsis-vertical"></i></button>
                                <div id="taskActions_${task.id}" class="private-dropdown-menu" style="display: none; position: absolute; ${isRTL ? 'left' : 'right'}: 0; top: 100%; min-width: 100px;">
            ...
            <div style="border-${isRTL ? 'right' : 'left'}: 4px solid ${moduleType.color}; background: var(--bg-surface); border-radius: 10px; margin-bottom: 20px; border: 1px solid var(--border-color); box-shadow: 0 2px 6px rgba(0,0,0,0.04);">
                <div style="padding: 14px; border-bottom: 3px solid ${moduleType.color}; display: flex; justify-content: space-between; align-items: center;">
                    ...
                    ${isAdmin ? `<div style="display: flex; gap: 4px; align-items: center;">
                        <button onclick="openSprintModal(null, '${moduleType.id}')" style="background: var(--success); border: none; color: #fff; width: 34px; height: 34px; border-radius: 8px; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center;" title="${state.currentLanguage === 'ar' ? 'سبرينت جديد' : 'New Sprint'}"><i class="fa-solid fa-plus"></i></button>
                        <button onclick="toggleModuleActivation('${moduleType.id}')" style="background: ${moduleType.isActive ? 'var(--success)' : 'var(--warning)'}; border: none; color: #fff; width: 34px; height: 34px; border-radius: 8px; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center;" title="${moduleType.isActive ? (isRTL ? 'إيقاف' : 'Deactivate') : (isRTL ? 'تفعيل' : 'Activate')}"><i class="fa-solid fa-power-off"></i></button>
                        <div style="position: relative; display: inline-block;">
                            <button onclick="event.stopPropagation(); toggleModuleActions('${moduleType.id}')" style="background: var(--primary); border: none; color: #fff; width: 34px; height: 34px; border-radius: 8px; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center;"><i class="fa-solid fa-ellipsis-vertical"></i></button>
    ...
    if(document.getElementById("statActiveTasks")) document.getElementById("statActiveTasks").innerText = state.tasks.filter(t => t.isActive !== false).length;
    if(document.getElementById("statInactiveTasks")) document.getElementById("statInactiveTasks").innerText = state.tasks.filter(t => t.isActive === false).length;
    ...
    if(document.getElementById("statActiveModules")) document.getElementById("statActiveModules").innerText = activeModules;
    if(document.getElementById("statInactiveModules")) document.getElementById("statInactiveModules").innerText = inactiveModules;
    ...
    if(document.getElementById("statActiveSprints")) document.getElementById("statActiveSprints").innerText = activeSprints;
    if(document.getElementById("statInactiveSprints")) document.getElementById("statInactiveSprints").innerText = inactiveSprints;
}

// ==========================================================================
// SPRINT INTEGRATION ENGINE (إدارة السبرينتات الشاملة + أزرار الإضافة والتعديل والحذف)
// ==========================================================================
 function openSprintFormModal(sprintId = null, boundModuleId = null) {
    if (!isCurrentUserAdmin()) return;
    document.getElementById("sprintId").value = sprintId || "";
    document.getElementById("sprintModuleId").value = boundModuleId || "";
    ...
    if (sprintId) {
        ...
        document.getElementById("sprintColor").value = boundModuleId ? (state.types.find(t => t.id === boundModuleId)?.color || "#4a6cf7") : "#4a6cf7";
    }
    const dict = state.currentLanguage === 'ar' ? { title: sprintId ? 'تعديل السبرينت' : 'سبرينت جديد' } : { title: sprintId ? 'Edit Sprint' : 'New Sprint' };
    document.getElementById("sprintModalTitle").innerHTML = `<i class="fa-solid fa-sprint"></i> ${dict.title}`;
    
    const today = new Date().toISOString().slice(0, 10);
    if (document.getElementById("sprintStartDate")) document.getElementById("sprintStartDate").min = today;
    if (document.getElementById("sprintEndDate")) document.getElementById("sprintEndDate").min = today;
...
function handleSprintSubmit(event) {
    ...
    if (!name) {
        showToast(state.currentLanguage === 'ar' ? 'يرجى إدخال اسم السبرينت' : 'Please enter sprint name', 'warning');
        return;
    }
    ...
    if (sprintId) {
        ...
        if (index > -1) {
            ...
            state.sprints[index].startDate = document.getElementById("sprintStartDate").value;
            state.sprints[index].endDate = document.getElementById("sprintEndDate").value;
...
function openTaskFormWizard(taskId = null, defaultModuleId = null, boundSprintId = null) {
    ...
    const moduleSelect = document.getElementById("taskBelongsToType");
    ...
    moduleSelect.value = defaultModuleId || (state.types[0] ? state.types[0].id : "");
    
    document.getElementById("taskSprintId").value = boundSprintId || "";
    ...
    if (taskId) {
        ...
        if (task) {
            ...
            const usersSelect = document.getElementById("taskAssignedUsers");
            if (usersSelect && task.assignedTo) {
                Array.from(usersSelect.options).forEach(opt => {
                    opt.selected = task.assignedTo.includes(opt.value);
                });
            }
            ...
            if (task.dueDate) dueDateInput.min = "";
        }
    } else {
...
function handleTaskSubmit(event) {
    ...
    const typeId = document.getElementById("taskBelongsToType").value || document.getElementById("taskSourceProjectId").value;
    const sprintId = document.getElementById("taskSprintId").value;
    const name = document.getElementById("taskName").value.trim();
    const usersSelect = document.getElementById("taskAssignedUsers");
    const selectedUsers = usersSelect ? Array.from(usersSelect.selectedOptions).map(o => o.value) : [];
    ...
    if (!name) {
        showToast(state.currentLanguage === 'ar' ? 'يرجى إدخال عنوان المهمة' : 'Please enter task title', 'warning');
        return;
    }
    
    // Check if user is selected for shared tasks
    if (!isPrivate && selectedUsers.length === 0) {
        showToast(state.currentLanguage === 'ar' ? 'يرجى تحديد مستخدم للمهمة' : 'Please select a user for the task', 'warning');
        return;
    }
    ...
    if (dueDate && !taskId) {
        ...
        if (taskDueDate < today) {
            showToast(state.currentLanguage === 'ar' ? 'تاريخ الاستحقاق لا يمكن أن يكون في الماضي' : 'Due date cannot be in the past', 'warning');
            return;
        }
    }
    
    // Check if due date is within sprint date range (for tasks with sprint)
    if (dueDate && sprintId && !taskId) {
        ...
        if (sprint && sprint.startDate && sprint.endDate) {
            ...
            if (taskDueDate < sprintStart || taskDueDate > sprintEnd) {
                showToast(state.currentLanguage === 'ar' ? 'تاريخ الاستحقاق يجب أن يكون بين تاريخ بدء السبرينت وانتهائه' : 'Due date must be within sprint date range', 'warning');
                return;
            }
        }
    }
    ...
    if (taskId) {
        ...
        if (isPrivate) {
            ...
            const index = vault.tasks.findIndex(t => t.id === taskId);
            if (index > -1) {
                vault.tasks[index].name = name;
                vault.tasks[index].typeId = typeId;
                vault.tasks[index].sprintId = sprintId || vault.tasks[index].sprintId;
                vault.tasks[index].dueDate = dueDate;
                vault.tasks[index].notes = notes;
                vault.tasks[index].tags = tags;
                vault.tasks[index].customColor = color;
            }
            ...
            if (index > -1) {
                ...
                state.tasks[index].assignedTo = selectedUsers;
                state.tasks[index].assignedUser = selectedUsers[0] || '';
        ...
        // إنشاء مهمة جديدة - بدون عمود todo، تبدأ مباشرة بقيد التكليف
        const newTask = {
            id: "tsk_" + Date.now(),
            typeId: typeId,
...
function toggleSprintActivation(sprintId) {
    ...
    if (module && module.isActive === false) {
        showToast(state.currentLanguage === 'ar' ? 'لا يمكن تفعيل السبرينت قبل تفعيل Project' : 'Cannot activate sprint before activating the project', 'warning');
        return;
    }
...
function initializeThemeEngine() {
    ...
    if (toggle) {
        toggle.addEventListener("click", () => {
            ...
            const nowTheme = currentAttr === 'dark' ? 'light' : 'dark';
            html.setAttribute("data-theme", nowTheme);
            state.systemThemes = nowTheme;
            updateThemeIcon(nowTheme);
            saveCoreRuntimeToStorage();
        });
    }
    if (state.systemThemes) {
        document.documentElement.setAttribute("data-theme", state.systemThemes);
        updateThemeIcon(state.systemThemes);
    }
}
...
function togglePasswordVisibility() {
    const input = document.getElementById("userAccessCode");
...
function openModal(id) { 
    ...
    // إظهار حقول المشروع الخاصة عند فتح المودال (للنوافذ التي تحتاجه)
    document.querySelectorAll(".private-task-project-element").forEach(el => el.style.display = "block");
}

function closeModal(id) { 
    if(document.getElementById(id)) document.getElementById(id).style.display = "none"; 
    // إظهار حقول المشروع الخاصة عند إغلاق المودال للنوافذ الأخرى
    document.querySelectorAll(".private-task-project-element").forEach(el => el.style.display = "block");
}
...
function toggleTaskActivation(taskId) {
    ...
    if (task.typeId) {
        ...
        if (module && module.isActive === false) {
            showToast(state.currentLanguage === 'ar' ? 'لا يمكن تفعيل المهمة قبل تفعيل Project' : 'Cannot activate task before activating the project', 'warning');
            return;
        }
    }
...
function toggleModuleActivation(moduleId) {
    ...
    if (!mod.isActive) {
        ...
        state.tasks.forEach(t => {
            if (t.typeId === moduleId && !t.sprintId) {
                t.isActive = false;
            }
        });
    }
    ...
    showToast(
        state.currentLanguage === 'ar'
            ? (mod.isActive ? `تم تفعيل Project "${mod.name}"` : `تم إلغاء تفعيل Project "${mod.name}"`)
