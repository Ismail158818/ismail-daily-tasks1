...
vaultContainer.innerHTML += `
             <div class="type-item-row" style="border-left-color: ${pType.color}; background: rgba(230, 126, 34, 0.04); border-radius: 8px; overflow: hidden;">
                 ...
                 <div style="padding: 12px; border-top: 1px solid var(--border-color);">
                     <button onclick="triggerPrivateTaskFormAllocation('${pType.id}')" style="width: 100%; background: ${pType.color}; border: none; color: #fff; padding: 8px; border-radius: 4px; cursor: pointer; font-size: 13px;"><i class="fa-solid fa-plus"></i> ${state.currentLanguage === 'ar' ? 'مهمة جديدة' : 'New Task'}</button>
                 </div>
             </div>
         `;
    });
...
function renderPrivateKanban() {
    ...
    ['todo', 'doing', 'done', 'rejected'].forEach(status => {
        ...
        tracks[status].forEach(task => {
            ...
            const isModuleActive = !type || type.isActive !== false;
            const taskSprint = vault.sprints ? vault.sprints.find(s => s.id === task.sprintId) : null;
...
// 3 نقط للمهام الخاصة (تعديل، حذف) - زر التفعيل منفصل
             let actionButtons = `
                 <div style="position: absolute; top: 8px; ${isRTL ? 'left' : 'right'}: 8px; display: flex; gap: 4px;">
                      <button onclick="event.stopPropagation(); ${isActivationAllowed ? `togglePrivateTaskActivation('${task.id}')` : ''}" ${!isActivationAllowed ? 'disabled' : ''} style="background: ${task.isActive ? 'var(--success)' : 'var(--warning)'}; border: none; color: ${isActivationAllowed ? '#fff' : '#999'}; width: 26px; height: 26px; border-radius: 4px; cursor: ${isActivationAllowed ? 'pointer' : 'not-allowed'}; font-size: 12px; opacity: ${isActivationAllowed ? 1 : 0.5};" title="${isActivationAllowed ? (task.isActive ? (state.currentLanguage === 'ar' ? 'إلغاء التفعيل' : 'Deactivate') : (state.currentLanguage === 'ar' ? 'تفعيل' : 'Activate')) : (state.currentLanguage === 'ar' ? 'المشروع أو السبرينت غير مفعل' : 'Project or sprint not active')}">
                         <i class="fa-solid fa-power-off"></i>
                     </button>
            ...
            column.innerHTML += `
                <div class="task-card-agile" data-id="${task.id}" style="border-top: 4px solid ${taskColor}; position: relative; cursor: pointer;" onclick="openTaskDetailModal('${task.id}')">
                    ...
                    <div class="task-footer-meta">
                        <span><i class="fa-solid fa-fingerprint"></i> ${(task.assignedTo && task.assignedTo.length > 0 ? task.assignedTo.join(', ') : (task.assignedUser || 'Unallocated'))}</span>
...
function togglePrivateTaskActivation(taskId) {
    const vaultKey = `taskvibe_vault_${state.currentUser}`;
    const vault = JSON.parse(localStorage.getItem(vaultKey));
    const task = vault.tasks.find(t => t.id === taskId);
...
// فتح نموذج إنشاء مهمة خاصة - يأخذ المشروع الذي تنتمي إليه تلقائيًا
function triggerPrivateTaskFormAllocation(typeId) {
    document.getElementById("taskForm").reset();
    document.getElementById("taskId").value = "";
    document.getElementById("taskSourceProjectId").value = typeId;
    
    document.getElementById("taskSprintId").value = "";
    document.getElementById("taskIsPrivate").value = "true";
    
    document.getElementById("taskModalTitle").innerText = state.currentLanguage === 'ar' ? 'إنشاء مهمة خاصة' : 'Create Private Task';
    
    // إخفاء عناصر المهام المشتركة
...
function openPrivateModuleModal(moduleId) {
    ...
    document.getElementById("typeModalTitle").innerHTML = '<i class="fa-solid fa-folder-open"></i> ' + (isRTL ? 'تعديل المشروع' : 'Edit Project');
...
function deletePrivateModule(moduleId) {
    ...
    const confirmText = isRTL ? `هل تريد حذف المشروع "${moduleType.name}" وجميع مهامه؟` : `Are you sure you want to delete Project "${moduleType.name}" and all its tasks?`;
    ...
    localStorage.setItem(vaultKey, JSON.stringify(vault));
    renderPrivateIsolatedWorkspace();
    showToast(isRTL ? 'تم حذف المشروع' : 'Project deleted', 'success');
}
...
function saveSprint(event) {
    ...
    if (startDate && new Date(startDate) < new Date().setHours(0,0,0,0)) {
        showToast(state.currentLanguage === 'ar' ? 'تاريخ البدء لا يمكن أن يكون في الماضي' : 'Start date cannot be in the past', 'warning');
        return;
    }
    
    // Check if end date is before start date
    if (startDate && endDate && new Date(endDate) < new Date(startDate)) {
        showToast(state.currentLanguage === 'ar' ? 'تاريخ الانتهاء لا يمكن أن يكون قبل تاريخ البدء' : 'End date cannot be before start date', 'warning');
        return;
    }
...
function createTaskForSprint(sprintId) {
    ...
    // تعبئة المشروع من السبرينت تلقائيًا
    const sprint = state.sprints.find(s => s.id === sprintId);
    document.getElementById("taskSourceProjectId").value = sprint ? sprint.typeId : "";
    
    // إخفاء حقل المشروع لأنه محدد من السبرينت
    document.querySelectorAll(".private-task-project-element").forEach(el => el.style.display = "none");
...
// رفض المهمة - نقلها إلى العمود المرفوض (سبب إلزامي)
function rejectTask(taskId) {
    ...
    state.tasks[taskIndex].rejectionReason = rejectionNote;
...
// نقل المهمة إلى عمود المرفوض (للأدمن فقط)
function moveTaskToRejected(taskId) {
    ...
    const promptText = state.currentLanguage === 'ar' ? 'سبب الرفض (إلزامي):' : 'Rejection reason (required):';
    const rejectionReason = prompt(promptText, '') || '';
    ...
    task.rejectedBy = state.currentUser;
    task.rejectedAt = now;
    task.updatedAt = now;
    ...
    showToast(state.currentLanguage === 'ar' ? 'تم نقل المهمة إلى المرفوض' : 'Task moved to Rejected', 'warning');
}
...
function deleteTask(taskId, isPrivate = false) {
    ...
    if (isPrivate) {
        ...
        if (vault) {
            const task = vault.tasks.find(t => t.id === taskId);
            if (!task) return;
            if (!isAdmin && !(task.assignedTo && task.assignedTo.some(u => (u || '').trim() === (state.currentUser || '').trim())) && (task.assignedUser || '').trim() !== (state.currentUser || '').trim()) {
...
function openModifyExistingTaskModal(taskId, isPrivateFlag) {
    ...
    if (isPrivateFlag && !isAdmin && !(task.assignedTo && task.assignedTo.some(u => (u || '').trim() === (state.currentUser || '').trim())) && (task.assignedUser || '').trim() !== (state.currentUser || '').trim()) {
        showToast(state.currentLanguage === 'ar' ? 'غير مسموح - ليس مهامك' : 'Access Denied - Not your task', 'danger');
        return;
    }
    ...
    if (isPrivateFlag) {
        // للمهام الخاصة، استخدم المشاريع من الخزنة الخاصة
        const vaultKey = `taskvibe_vault_${state.currentUser}`;
        const vault = JSON.parse(localStorage.getItem(vaultKey) || '{"types":[],"tasks":[]}');
        vault.types.forEach(m => {
    ...
    moduleSelect.value = task.typeId || "";
    ...
    if (!isPrivateFlag) {
        ...
        if (usersSelect) {
            const users = task.assignedTo || (task.assignedUser ? [task.assignedUser] : []);
...
function triggerTaskFormAllocation(typeId, isPrivateFlag) {
    ...
    const dict = state.currentLanguage === 'ar' ? {
        privateTitle: 'إنشاء مهمة خاصة',
        sharedTitle: 'إنشاء مهمة مشتركة'
    } : {
        privateTitle: 'Create Private Task',
        sharedTitle: 'Create Shared Task'
    };
    ...
    // إخفاء حقل المشروع لأنه محدد من البطاقة
    document.querySelectorAll(".private-task-project-element").forEach(el => el.style.display = "none");
...
function editModule(moduleId) {
    ...
    document.getElementById("typeModalTitle").innerHTML = '<i class="fa-solid fa-folder-open"></i> ' + (state.currentLanguage === 'ar' ? 'تعديل المشروع' : 'Edit Project');
...
function deleteModule(moduleId) {
    ...
    const confirmText = state.currentLanguage === 'ar'
        ...
        : `Are you sure you want to delete Project "${moduleType.name}" and all its tasks?`;
    ...
    showToast(state.currentLanguage === 'ar' ? 'تم حذف المشروع' : 'Project deleted', 'success');
}

// فتح تفاصيل المشروع (مشاريع مشتركة أو خاصة)
